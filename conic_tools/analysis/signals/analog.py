...
class AnalogSignalList(object):
    ...
    def __init__(self, signals, id_list, dt=None, times=None, t_start=None, t_stop=None, dims=None):
        ...
        self.dt = np.round(float(dt), operations.determine_decimal_digits(dt))

        if t_start is None and times is None:
            t_start = 0.
        elif t_start is None and times is not None:
            t_start = min(times)
            if abs(np.round(min(times) - np.round(min(times)), operations.determine_decimal_digits(self.dt))) <= self.dt:
                    t_start = np.round(min(times), operations.determine_decimal_digits(self.dt))
        ...
        if t_stop is None and times is None:
            ...
            t_stop = max(times)
            if abs(np.round(max(times) - np.round(max(times)), operations.determine_decimal_digits(self.dt))) <= self.dt:
                t_stop = np.round(max(times), operations.determine_decimal_digits(self.dt))
                if t_stop == max(times) and self.dt >= 1.:
                    t_stop += self.dt
        self.t_stop = round(t_stop, operations.determine_decimal_digits(self.dt))
    ...
    def zero_pad(self, n_steps=10):
        ...
        stim_array = pad_array(self.as_array(), add=int(n_steps))
    ...
    def plot(self, as_array=False, fig=None, ax=None, display=True, save=False):
        ...
        if not as_array:
            ...
            plot_matrix(self.as_array(), ax=ax, display=True)
        fig_output(fig, display, save)
